...
output: 
    flexdashboard::flex_dashboard:
        orientation: rows
params:
    indir: "/Users/prados/Documents/AMR-genomics/amr-tools/work/1a/efb6b0fd83cc3a7bb15b1ed0d77e91"
---
...
amrfinderplus <- db$assemblies |>
	...
	bind_rows(tibble(Class=character(0),Subclass=character(0),resistance_name=character(0)))
bind_rows(
	 amrfinderplus |>
			group_by(
				id=str_c(Class),
...
```{r}
assemblies |>
	select(species_name,mlst_type,num_contig,assembly_length,GC,N50,orgfinder.species_name,orgfinder.ANI) |>
	mutate(GC = 100*GC) |>
	reactable(
		pagination = FALSE,
		compact = TRUE,
...
summarise_resistances(db) |>
	distinct(sample_id,resistance_name) |>
	mutate(resistance_name = fct_reorder2_hclust(resistance_name,sample_id,rep(1,n()))) |>
...
p2 <- CONTIGS |>
	select(contig_id=id,resistance_names) |>
	unnest_longer(resistance_names) |> 
	ungroup() |>
	distinct() |>
	mutate(resistance_names=fct_reorder2_hclust(resistance_names,contig_id,rep(1,n()))) |>
	ggplot(aes(x=contig_id,y=resistance_names)) +
	geom_tile() +
