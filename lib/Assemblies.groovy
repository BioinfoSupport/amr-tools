...
class Assemblies {
	...
	static def fromCSV(csv_path,samplesheetToList) {
		return Channel.fromList(samplesheetToList(csv_path,"assets/schema/sheets/assemblies.json"))
			.map({it[0].sample_id = it[0].sample_id?:it[0].readset_id;it[0]})
	}
	...
	static def long_reads_channel(readsets) {
		return readsets
			...
			.map({[it.subMap('readset_id','sample_id'),it.long_reads]})
	}

	static def short_reads_channel(readsets) {
		return readsets
			.filter({it.short_reads_1})
			.map({[it.subMap('readset_id','sample_id'),[it.short_reads_1,it.short_reads_2].findAll({it})]})
	}
}
